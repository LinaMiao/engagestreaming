...
object EngageStreaming {
  ...
  def main(args: Array[String]) {
    // topics from kafka
    val brokers = "ec2-34-212-24-253.us-west-2.compute.amazonaws.com:9092"
    ...
    val topicsSet_7 = topics_6.split(",").toSet
    ...
    val windowStream_1 = messages_1.window(Seconds(30), Seconds(30))
    ...
    windowStream_1.foreachRDD { rdd =>
        ...
        val sentiments = rdd.map(_._2)
                            .map(_.split(","))
                            .map(t => (t(0).toString()(1).toString(), t(1).toString()))
                            .toDF("id", "comment")
        ...
        val r1 = new RedisClient("54.201.180.66", 6379, database=1, secret=Option("127001"))
        sentiments.collect().foreach( t => {r1.set(t(0), t(1).toString())})
                            }
      ...
      windowStream_2.foreachRDD { rdd =>
          ...
          val likesCount  = rdd.map(_._2)
                              .map(_.split(","))
                              .map(t => (t(0).toString()(1).toString(), 1))
                              .toDF("id","likes")
                              .groupBy("id")
                              .count()

          val r2 = new RedisClient("54.201.180.66", 6379,database=2,secret=Option("127001"))
      ...
      windowStream_3.foreachRDD { rdd =>
          ...
          val r3 = new RedisClient("54.201.180.66", 6379,database=3,secret=Option("127001"))
      ...
      windowStream_4.foreachRDD { rdd =>
          ...
          val starts = rdd.map(_._2)
                              .map(_.split(","))
                              .map(t => (t(0).toString()(1).toString(), t(0).toString() + t(1).toString() + t(2).toString() + t(3).toString() + t(4).toString() + t(5).toString() + t(6).toString()))
                              .toDF("id","content")



          val r4 = new RedisClient("54.201.180.66", 6379,database=4,secret=Option("127001"))
    ...
    windowStream_5.foreachRDD { rdd =>
        ...
        val r5 = new RedisClient("54.201.180.66", 6379,database=5,secret=Option("127001"))
      ...
      val mappingFunc_6 = (id: String, one: Option[Int], state:State[Int]) => {
      ...
      playStateCount.foreachRDD { rdd =>
          ...
          val r6 = new RedisClient("54.201.180.66", 6379,database=6,secret=Option("127001"))
          playCount.collect().foreach( t => {r6.set(t(0).toString()(1).toString(), t(1))})

                              }


      // DStream for topic7, plays, with sliding window 10, 10
      ...
      val mappingFunc_7 = (id: String, one: Option[Int], state:State[Int]) => {
        val sum = one.getOrElse(0) + state.getOption.getOrElse(0)
        val output = (id, sum)
        state.update(sum)
        output
      ...
      leaveStateCount.foreachRDD { rdd =>
          ...
          val r7 = new RedisClient("54.201.180.66", 6379, database=7,secret=Option("127001"))
          leaveCount.collect().foreach( t => {r7.set(t(0).toString()(1).toString(), t(1))})
